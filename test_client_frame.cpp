...
namespace testosteron
{
	...
	void TestClientFrame::setupTitlePanel()
	{
	...
	void TestClientFrame::setupOutputInfoPanel()
	{
	...
	void TestClientFrame::setupFailedFunctionsInfoPanel()
	{
	...
	void TestClientFrame::setupFailedFunctionsListBox()
	{
	...
	void TestClientFrame::setupTestFunctionsInfoPanel()
	{
	...
	void TestClientFrame::setupTestFunctionsListBox()
	{
		...
		testFunctionsListBox.Clear();

		for (unsigned int index = 0; index < test.clientTest().size(); index++)
		{

			testFunctionsListBox.AppendString
			(test.clientTest().at(index).getFunctionName());

		}

	}

	void TestClientFrame::setupButtons()
	{
	...
	void TestClientFrame::setupAuthorInfo()
	{
	...
	void TestClientFrame::activateButtons()
	{

		testButton.Bind
		(wxEVT_BUTTON, &TestClientFrame::onClickTest, this);

		exitButton.Bind
		(wxEVT_BUTTON, &TestClientFrame::onClickExit, this);

	}

	void TestClientFrame::onClickTest(wxCommandEvent& event)
	{
		...
		for (unsigned int index = 0; index < test.clientTest().size(); index++)
		{
			...
			if (!test.clientTest().at(index).hasPassed())
			{
		...
		output.SetLabel
		(test.specializedTestOutput(test.clientTest(), "Client"));
	...
	void TestClientFrame::onClickExit(wxCommandEvent& event)
	{
	...
	void TestClientFrame::activateFailedFunctionsListBox()
	{

		failedFunctionsListBox.Bind
		(wxEVT_LISTBOX, &TestClientFrame::onClickShowInfo, this);

	}

	void TestClientFrame::onClickShowInfo
